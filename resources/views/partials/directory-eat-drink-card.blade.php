@php
  ...
  $logo = function_exists('get_field') ? get_field('eat_drink_logo', $post_id) : null;
  ...
  $hours_raw = function_exists('get_field') ? get_field('eat_drink_hours_summary', $post_id) : '';
  $hours = is_string($hours_raw) && trim($hours_raw) !== '' ? trim($hours_raw) : __('Opening hours TBC', 'culvers');
  $cat_slugs = wp_get_post_terms($post_id, 'culvers_eat_drink_category', ['fields' => 'slugs']);
  $type_slugs = wp_get_post_terms($post_id, 'culvers_eat_drink_type', ['fields' => 'slugs']);
