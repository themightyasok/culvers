@php
  /**
   * Event card — used on /latest-events/ and anywhere a
   * `culvers_event` needs to render in the directory grid (three_card_block,
   * related events, etc.).
   *
   * Mirrors {@see directory-shop-card.blade.php} structurally so every
   ...
   *   • Bottom title   → event title
   *   • Bottom subtitle→ "date · time" pair, or location, falling back to either alone
   *
   * @var \WP_Post $post
   */
  ...
  $date_raw = function_exists('get_field') ? get_field('event_card_date', $post_id) : '';
  $time_raw = function_exists('get_field') ? get_field('event_card_time', $post_id) : '';
  $location_raw = function_exists('get_field') ? get_field('event_card_location', $post_id) : '';

  $date = is_string($date_raw) ? trim($date_raw) : '';
  $time = is_string($time_raw) ? trim($time_raw) : '';
  $location = is_string($location_raw) ? trim($location_raw) : '';

  /* Subtitle priority: date · time > date alone > time alone > location.
     Cards are tight so we surface the most date-anchored line first; venues
     usually live in the title context and are clear from the hero photo. */
  if ($date !== '' && $time !== '') {
      $subtitle = $date . ' · ' . $time;
  } elseif ($date !== '') {
      $subtitle = $date;
  ...
  $cat_terms = wp_get_post_terms($post_id, 'culvers_event_category');
