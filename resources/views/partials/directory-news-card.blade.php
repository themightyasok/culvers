@php
  /**
   * News article card — used on /latest-news/ and anywhere a
   * `culvers_news` post needs to render in the directory grid.
   *
   * Mirrors {@see directory-offer-card.blade.php} structurally so every
   * directory archive (shop / eat-drink / events / offers / news / careers)
   * reads as siblings: 294px-tall moss block (logo slot ↔ feature photo on
   * hover), white separator, title + subtitle band.
   *
   * News mapping:
   *   • Logo slot      → eyebrow override OR primary News category term name
   *   • Hover photo    → post thumbnail
   *   • Bottom title   → article title
   *   • Bottom subtitle→ display publish date (ACF override or post date)
   *
   * @var \WP_Post $post
   */
  ...
  $eyebrow_raw = function_exists('get_field') ? get_field('news_card_eyebrow', $post_id) : '';
  $eyebrow = is_string($eyebrow_raw) ? trim($eyebrow_raw) : '';

  $cat_terms = wp_get_post_terms($post_id, 'culvers_news_category');
  ...
  /* Eyebrow override → primary category → post type label fallback. */
  if ($eyebrow === '') {
      $eyebrow = $primary_category_name !== '' ? $primary_category_name : __('News', 'culvers');
  }

  $publish_override_raw = function_exists('get_field') ? get_field('news_card_published_on', $post_id) : '';
  $publish_override = is_string($publish_override_raw) ? trim($publish_override_raw) : '';
  $subtitle = $publish_override !== ''
      ? $publish_override
  ...
  <a
    ...
    <div class="relative h-[294px] w-full bg-dustleaf">
      ...
      <div
        ...
        <span class="text-center font-heading text-2xl font-medium text-white">{{ esc_html($eyebrow) }}</span>
      </div>
