@php
  ...
  $footerLegalListClass =
      'footer-nav__list footer-nav__list--legal flex flex-wrap items-center justify-center gap-x-[18px] gap-y-2 whitespace-normal font-label text-[10px] font-normal uppercase leading-[24px] tracking-[0.5px] text-lighter-cream [&>li]:flex [&>li]:shrink-0 [&>li]:items-center [&>li>a]:inline-flex [&>li>a]:min-h-[2rem] [&>li>a]:items-center [&>li>a]:px-0.5 [&>li>a]:py-1';

  $footerMenuListClass =
      'footer-nav__list footer-nav__list--menu flex flex-col [&>li]:pb-[2px]';
    ...
    <div class="site-footer__pattern" aria-hidden="true">
      <picture>
        <source
          ...
          srcset="{{ esc_url(get_theme_file_uri('resources/images/footer/footer-background-desktop.svg')) }}" />
        <img
          src="{{ esc_url(get_theme_file_uri('resources/images/footer/footer-background-mobile.svg')) }}"
          alt=""
          width="430"
          height="971"
          decoding="async"
    ...
    <div class="relative z-10 mx-auto w-full max-w-8xl">
      ...
      <section
        ...
        <div
          ...
          <div class="relative z-10 grid min-h-[300px] grid-cols-1 items-center gap-8 px-6 py-11 max-lg:min-h-[604px] md:min-h-[380px] md:grid-cols-2 md:gap-10 md:px-12 md:py-14 lg:min-h-[420px] lg:gap-14 lg:px-14 lg:py-16">
            ...
            <div
              ...
              <div class="w-full max-w-[26rem] text-center md:mx-0">
                <h2
                  ...
                  class="w-full text-center font-heading text-[36px] font-normal leading-[1.1] text-white md:text-4xl">
              ...
              <form
                class="footer-newsletter-form pointer-events-auto w-full max-w-[313px] shrink-0 max-lg:max-w-[313px] md:max-w-[363px]"
                method="post"
                ...
                <div
                  ...
                  <input
                    ...
                    class="min-w-0 flex-1 border-0 bg-transparent font-label text-[13px] font-normal leading-8 tracking-[0.64px] text-white placeholder:font-semibold placeholder:uppercase placeholder:tracking-[0.64px] placeholder:text-white placeholder:opacity-100 focus:ring-0 focus:outline-none md:text-xs md:tracking-widest md:placeholder:tracking-widest" />
                  <button
                    type="submit"
      ...
      <div class="relative z-10 grid grid-cols-1 gap-12 pt-[55px] lg:grid-cols-4 lg:gap-10 lg:pt-[40px] xl:gap-14">
        <div class="flex flex-col">
          ...
          @if($addr !== '')
            ...
            <div class="footer-sans-book mt-[14px] text-[20px] leading-[30px] text-light-cream">
              {!! nl2br(esc_html($addr)) !!}
            </div>
          @endif
          @if($mapUrl !== '')
            ...
            <a
              href="{{ esc_url($mapUrl) }}"
              class="footer-getting-here-map-link mt-6 inline-flex max-w-full w-max shrink-0 self-start flex-nowrap items-center gap-2 border-b-2 border-glowleaf pb-0.5 font-label text-[14px] font-semibold uppercase leading-[24px] tracking-[1px] text-glowleaf transition-colors hover:border-lighter-cream hover:text-lighter-cream"
        ...
        <div class="flex flex-col">
          ...
          @if($phone !== '')
            <a
              class="footer-nav__link-phone mt-[14px] block"
          ...
          @if($contactEmail !== '')
            <a
              class="footer-link--persistent-underline footer-sans-book {{ $phone !== '' ? 'mt-0' : 'mt-[14px]' }} block w-fit text-[20px] leading-[30px] text-glowleaf underline transition-colors hover:text-lighter-cream"
              href="{{ esc_url('mailto:' . $contactEmail) }}">
              {{ esc_html($contactEmail) }}
            </a>
          @endif
          @if(($instagramUrl !== '' && $instagramUrl !== '#') || ($facebookUrl !== '' && $facebookUrl !== '#'))
            <div class="footer-social-row mt-[24px] flex flex-wrap gap-x-[30px] gap-y-3 lg:flex-nowrap lg:gap-y-0">
              @if($instagramUrl !== '' && $instagramUrl !== '#')
                <a
                  ...
                  @include('partials.figma-social-icon', [
                      ...
                      'social_icon_class' => 'size-[16px] shrink-0 overflow-visible text-current',
                  ])
              ...
              @if($facebookUrl !== '' && $facebookUrl !== '#')
                <a
                  ...
                  @include('partials.figma-social-icon', [
                      ...
                      'social_icon_class' => 'size-[17px] shrink-0 text-current',
                  ])
                  {{ __('Facebook', 'culvers') }}
                </a>
              @endif
        ...
        <div class="hidden flex-col lg:flex">
          ...
          @elseif(current_user_can('edit_theme_options'))
            <p class="mt-[14px] font-sans text-base text-light-cream/55">
        ...
        <div class="flex flex-col">
          <button
            ...
            <span class="flex w-full items-center justify-between gap-4">
              <span class="font-heading text-[32px] leading-[1.1] text-lighter-cream">
                {{ esc_html(FooterCustomizer::columnOneTitle()) }}
              </span>
              @include('partials.footer-accordion-toggle', ['expandedExpression' => 'openWhatsHere'])
            </span>
            <span class="mt-2 block w-full border-b border-glowleaf" aria-hidden="true"></span>
          </button>
          @if($hasFooterMenuWhatsHere)
            <nav
        ...
        <div class="flex flex-col">
          <button
            ...
            <span class="flex w-full items-center justify-between gap-4">
              ...
              @include('partials.footer-accordion-toggle', ['expandedExpression' => 'openUsefulLinks'])
            </span>
            <span class="mt-2 block w-full border-b border-glowleaf" aria-hidden="true"></span>
          </button>
          @if($hasFooterMenuUsefulLinks)
            ...
            if (is_readable($footerWordmarkAbs)) {
                ...
                if ($footerWordmarkRaw !== '') {
                    $footerWordmarkSvg = (string) preg_replace(
                        '/<svg\b/',
                        '<svg class="block h-auto w-full max-w-none text-glowleaf lg:mb-10" aria-hidden="true" focusable="false"',
                        $footerWordmarkRaw,
                        1,
                    );
                }
      ...
      <div class="footer-under-logo relative z-20 w-full border-t border-glowleaf max-lg:mt-10 lg:mt-0">
        <div
          ...
          <p
            class="hidden font-label text-[10px] font-normal uppercase leading-[24px] tracking-[0.5px] text-lighter-cream lg:block lg:justify-self-start lg:text-left">
            &copy;
            {{ esc_html(strtoupper((string) get_bloginfo('name'))) }}
            {{ wp_date('Y') }}
          </p>
          ...
          @if($credit !== '')
            <p
              class="font-label text-[12px] font-normal leading-[17px] tracking-[0.5px] text-lighter-cream max-lg:text-[12px] max-lg:leading-[17px] md:justify-self-end md:text-right lg:text-[10px] lg:leading-[24px]">
              {{ esc_html($credit) }}
            </p>
          @elseif($societyLogoSvg !== '')
            <div
              class="flex flex-wrap items-center justify-start gap-x-2 gap-y-1 text-left font-label max-lg:justify-self-start md:justify-self-end md:justify-end">
              {{-- Figma `51:5146`: Off White label + Society Studios wordmark (Layer_1). --}}
              <span
                class="text-[12px] font-normal uppercase leading-[17px] tracking-[0.5px] text-purelinen max-lg:text-[12px] max-lg:leading-[17px] lg:text-[10px] lg:leading-[24px]">
                {{ __('Site by', 'culvers') }}
              </span>
