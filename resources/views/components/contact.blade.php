@php
  ...
  $afterSubmit = trim((string) ($c['contact_after_submit'] ?? ''));
    ...
    <div class="contact__band grid gap-10 md:gap-12 @if($showPanel) lg:grid-cols-[minmax(0,30%)_minmax(0,1fr)] lg:gap-16 @endif">
      ...
      <form
        ...
        novalidate>
          ...
          @if($afterSubmit !== '')
            <div
              class="contact__after-submit md:col-span-2 mt-8 max-w-xl font-sans text-base font-light leading-[1.45] text-faded-olive [&_h2]:mb-3 [&_h2]:font-heading [&_h2]:text-2xl [&_h2]:font-normal [&_h2]:leading-tight [&_h2]:text-deep-moss [&_h3]:mb-3 [&_h3]:font-heading [&_h3]:text-2xl [&_h3]:font-normal [&_h3]:leading-tight [&_h3]:text-deep-moss [&_p]:mt-3 [&_p:first-child]:mt-0 [&_a]:underline [&_a]:decoration-faded-olive/40 [&_a]:underline-offset-2 hover:[&_a]:decoration-dustleaf [&_strong]:font-medium [&_strong]:text-deep-moss">
              {!! wp_kses_post($afterSubmit) !!}
            </div>
          @endif
        </form>
    </div>
