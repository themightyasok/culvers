...
@if($hasDetails)
  <section
    ...
    <div class="{{ LayoutShell::INNER_READABLE_960 }}">
      {{-- Figma: section title Canela 58px; column titles H4 32px / lh 1.1; values Halyard Book 24px / lh 30 — colours #4f5438. --}}
      <{{ $headingTag }} class="mb-10 text-center font-heading text-[58px] leading-[1.15] tracking-tight text-faded-olive md:mb-12">
      ...
      <div
        ...
        <div class="flex flex-col items-center text-center lg:px-8 lg:pb-0 lg:pt-1 {{ $hasSocial ? '' : 'lg:pl-0' }}">
          <p class="font-heading text-2xl leading-[1.1] text-faded-olive">{{ esc_html($contactLabel) }}</p>
          @if($phone !== '')
            @php $telHref = preg_replace('/[^0-9+]/', '', str_replace("\xc2\xa0", ' ', $phone)); @endphp
            <p class="mt-3 font-sans text-xl font-light leading-[30px] text-faded-olive {{ esc_attr($tone) }}">
              @if($telHref !== '')
        ...
        <div class="flex flex-col items-center pt-10 text-center lg:px-8 lg:pt-1">
          <p class="font-heading text-2xl leading-[1.1] text-faded-olive">{{ esc_html($addressLabel) }}</p>
        ...
        @if($hasSocial)
          <div class="flex flex-col items-center pt-10 text-center lg:px-8 lg:pt-1 lg:pr-0">
            <p class="font-heading text-2xl leading-[1.1] text-faded-olive">{{ esc_html($socialLabel) }}</p>
            <div class="mt-4 flex items-center justify-center gap-3 font-sans">
              <span class="inline-flex size-9 shrink-0 items-center justify-center rounded-full border border-faded-olive/20 bg-white/40 text-faded-olive" aria-hidden="true">
              ...
              @if($igUrl !== '')
                <a
                  class="text-xs font-semibold uppercase tracking-[1px] text-faded-olive underline decoration-brand-500 underline-offset-4 hover:decoration-faded-olive"
                  href="{{ esc_url($igUrl) }}"
              ...
              @else
                <span class="text-xs font-semibold uppercase tracking-[1px] text-faded-olive">{{ esc_html($igHandle) }}</span>
              @endif
            </div>
          </div>
        @endif
